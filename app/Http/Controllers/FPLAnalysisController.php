...
class FPLAnalysisController extends Controller
{
    ...
    public function dashboard(Request $request)
    {
        try {
            ...
            $data = [
                ...
                'captain_recommendations' => collect($this->queryService->getCaptainRecommendations($currentGameweek + 1))->take(10),
                'differential_players' => collect($this->queryService->getDifferentialPlayers(10.0, 40))->take(8),
                'transfer_recommendations' => collect($this->queryService->getTransferRecommendations())->take(10),
            ];
     ...
     */
    public function captainRecommendations(Request $request): JsonResponse
    {
        try {
            $gameweek = $request->get('gameweek', 1);
            $data = $this->queryService->getCaptainRecommendations($gameweek);
    ...
    public function differentialPlayers(Request $request): JsonResponse
    {
        try {
            $maxOwnership = $request->get('max_ownership', 10.0);
            $minPoints = $request->get('min_points', 50);
            
            $data = $this->queryService->getDifferentialPlayers($maxOwnership, $minPoints);
    ...
    public function cleanSheetProbabilities(Request $request): JsonResponse
    {
        try {
            $gameweek = $request->get('gameweek', 1);
            $data = $this->queryService->getCleanSheetProbabilities($gameweek);
    ...
    public function transferRecommendations(): JsonResponse
    {
        try {
            $data = $this->queryService->getTransferRecommendations();
    ...
    public function fixtureDifficulty(Request $request): JsonResponse
    {
        try {
            $fromGameweek = $request->get('from_gameweek', 1);
            $gameweeks = $request->get('gameweeks', 5);
            
            $data = $this->queryService->getTeamFixtureDifficulty($fromGameweek, $gameweeks);
